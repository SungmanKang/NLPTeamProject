...
def _load_w2v_model_gensim(filename):
    ...
    logger.info('HMMLoading embeddings from %s', filename)
...
def _load_w2v_model_file(filename):
    ...
    logger.info('RYANRYANRYANLoading embeddings from %s', filename)
    w2v = DummyDict()
        ...
        for line in f:
            ...
            if skip:
                ...
                while len(RECline) < 768:
                    RECline.append(0.0)
                #logger.info('RYAN')
                #logger.info(len(RECline))
                #logger.info(np.array([np.float(entry) for entry in line[index + 1:].split()]).shape)
