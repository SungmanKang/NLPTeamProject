...
from transformers import BertModel, BertConfig, BertTokenizer, BertPreTrainedModel, PreTrainedTokenizer, PreTrainedModel
from transformers import RobertaModel, RobertaTokenizer, RobertaConfig #Roberta imports
#from transformers import DebertaConfig, DebertaModel, DebertaTokenizer #DeBERTa imports
...
class Bertram(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        if target_vectors is not None:
            loss_fct = MSELoss()
            #if output_vectors.shape != target_vectors.shape:
                #output_vectors = torch.FloatTensor(target_vectors)
                #output_vectors.reshape(target_vectors.shape)
...
#class BertramForDeBERTa(Bertram):
#    """An instance of BERTRAM that used RoBERTa instead of BERT as the underlying language model"""
#    config_class = DebertaConfig
#    base_model_prefix = "deberta"

#    def __init__(self, transformer_config: BertConfig, bertram_config: BertramConfig, do_setup=False):
#        super(BertramForDeBERTa, self).__init__(transformer_config, bertram_config, do_setup)


MODELS = {
    'bert': (BertModel, BertTokenizer, Bertram),
    'roberta': (RobertaModel, RobertaTokenizer, BertramForRoberta)######################,
    #'deberta': (DebertaModel, DebertaTokenizer, BertramForDeBERTa)
}
